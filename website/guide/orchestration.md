...
`ScriptGenerator` converts an `OrchestrationPlan` into a standalone C# program that connects to the Orleans cluster and executes each step sequentially.
...
PlanningAgent:
  ...
  OrchestrationCompiler.Compile(script, references)  // optional validation
     |
     v
  ScriptExecutor.ExecuteScriptAsync(script, workingDir)
     |
     v
  ...
  Script connects to cluster, invokes agents in order:
    Step 1: DotNet.SendMessage("Run all tests")
    Step 2: Reviewer.SendMessage("Review test results")
    Step 3: Deployer.SendMessage("Deploy to staging")
```
